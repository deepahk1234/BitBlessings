...
final class ChatViewModel: ObservableObject, BitchatDelegate, CommandContextProvider, GeohashParticipantContext, MessageFormattingContext {
    ...
    func didReceivePublicMessage(from peerID: PeerID, nickname: String, content: String, timestamp: Date, messageID: String?) {
        Task { @MainActor in
            ...
            // ── Blessings protocol intercept ──────────────────────────────────────
            // Parse BLESSING_Q and BLESSING_R messages without disrupting normal chat.
            if normalized.hasPrefix(BlessingMessageProtocol.questionPrefix) ||
               normalized.hasPrefix(BlessingMessageProtocol.responsePrefix) {
                BlessingsService.shared.handleIncomingMessage(from: nickname, content: normalized)
                return   // Blessing protocol messages are not shown in the public chat timeline
            }
            // ─────────────────────────────────────────────────────────────────────
